...
class FFL_Admin_Theme_Login
{
	...
	function __construct($setting)
	{
		...
		add_action('admin_screen_login_section', array($this, 'admin_screen'));
	...
	function login_css()
	{
		...
		if ($bg_color) {
			$css .= sprintf('body{background-color: %s}', $bg_color);
		}

		$bg_img = $this->setting->get_setting('login_bg_img');
		if ($bg_img) {
			$css .= sprintf('body{background-image: url( %s )}', $bg_img);
		}

		$left_bg_color = $this->setting->get_setting('login_left_bg_color');
		if ($left_bg_color) {
			$css .= sprintf('#login{background-color: %s}', $left_bg_color);
		}

		$left_bg_img = $this->setting->get_setting('login_left_bg_img');
		if ($left_bg_img) {
			$css .= sprintf('#login{background-image: url( %s )}', $left_bg_img);
		}

		$text_color = $this->setting->get_setting('login_text_color');
		if ($text_color) {
			$css .= sprintf('body{color: %s}', $text_color);
		}

		$logo = $this->setting->get_setting('login_logo');
		if ($logo) {
			$css .= sprintf('body.login div#login > h1{background-image: url( %s )} body.login div#login h1 a {background-image: none;}', $logo);
		}
